...
- Core now builds those actions for decision events (`session.needs_input`,
  `review.changes_requested`, `merge.ready`) and routes them through
  `notifyWithActions` when a notifier supports it. Each button is an HMAC-signed,
  expiring token minted with the shared `AO_NOTIFY_CALLBACK_SECRET`; without the
  secret set, notifications behave exactly as before (opt-in). New core exports:
  `buildNotifyActions`, `signCallbackToken`, `verifyCallbackToken`,
  `getNotifyCallbackSecret`, `isNotifyActionEvent`, and the
  `NOTIFY_CALLBACK_*` constants.
