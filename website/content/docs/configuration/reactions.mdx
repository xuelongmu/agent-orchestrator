...
| `confidenceThreshold` | `number` (0..1) | Minimum heuristic confidence to run the autonomous action; below it AO holds and escalates to a human with a question |
...
Set `confidenceThreshold` on a reaction with an `auto-merge` or `send-to-agent`
(auto-fix) action to require human sign-off when AO is not confident enough. Before
acting, AO folds cheap risk signals — review-round churn, the severity and confidence
of open automated review findings, CI failures on the branch, and diff size across the
session's PRs — into a heuristic score between 0 and 1. When the score is below the
threshold, the action is **held** and escalated to a human with a question explaining
why, instead of running. Omit the field to keep the reaction's original behavior.

(`spawn-session` is not gated: the dependency scheduler launches held dependents
independently each poll, so the reaction-level hold could not stop the spawn.)

```yaml
reactions:
  approved-and-green:
    auto: true # approved-and-green defaults to auto: false — enable it
    action: auto-merge
    confidenceThreshold: 0.75 # hold auto-merge below 75% confidence
```

Workers can also hand a judgment call up explicitly:

```bash
ao report needs-decision --confidence 0.4 --question "Drop the legacy column or keep it?"
```

This parks the session in `needs_input` and surfaces the confidence and question to the
human, so the agent never guesses on a risky call it is unsure about.
