...
/**
 * The SINGLE source of truth for an active agent decision (Class A chokepoint).
 * A `needs_decision` report is active ONLY while the session is parked in
 * needs_input: once lifecycle inference (or a later `ao report`) moves it out, the
 * decision is resolved and clearStaleDecisionContext clears the question, so this
 * returns null. Every consumer reads through here — no site re-infers or copies
...
/** Auto-merge never runs without an explicit confidence floor. */
const DEFAULT_AUTO_MERGE_CONFIDENCE_THRESHOLD = 0.8;
...
function requireSuccessfulSCMRefresh(outcomes: SCMRefreshOutcomes, session: Session): void {
...
export function createLifecycleManager(deps: LifecycleManagerDeps): LifecycleManager {
  ...
  async function populatePREnrichmentCache(
    ...
    for (const session of sessions) {
      ...
      sessionKeysByPlugin.get(pluginKey)?.add(webhookSessionKey(session.projectId, session.id));
    ...
    for (const [pluginKey, pluginPRs] of prsByPlugin) {
      ...
      try {
        ...
        recordSCMRefreshSuccess(scmRefreshOutcomes, sessionKeysByPlugin.get(pluginKey) ?? []);
      } catch (err) {
        ...
        recordSCMRefreshFailure(scmRefreshOutcomes, sessionKeysByPlugin.get(pluginKey) ?? [], err);
      }
    }
  ...
  function gatherConfidenceSignals(session: Session): ConfidenceSignals {
    ...
    // CI-failure signal (#12 review). Take the max of:
    //  • the live ci-failed tracker attempts (resets on each new fingerprint),
    //  • the CUMULATIVE failure count, which survives CI recovery until a terminal
    //    PR state — so a PR that failed CI then went green before approved-and-green
    //    still scores that churn, and
    //  • 1 whenever CI is failing right now (a same-state failure the tracker missed).
    const ciTracker = reactionTrackers.get(`${session.id}:ci-failed`);
    const ciFailureCount = Math.max(
      ciTracker?.attempts ?? 0,
      parseAttemptCount(session.metadata["ciFailureCountTotal"]),
      ciCurrentlyFailing ? 1 : 0,
    );
    ...
    try {
      ...
      for (const finding of unresolvedFindings) {
        ...
        const isRiskier =
          ...
          (findingRank === currentRank && (finding.confidence ?? 1) > (maxFindingConfidence ?? 1));
        if (isRiskier) {
  ...
  /**
   * Deliver the confidence escalation for a held action: the confidence question,
   * plus (when provided) the content that would have gone to the agent — enriched
   * review comments / CI details — so a single notification is actionable. Returns
   * whether a human actually received it.
   */
  async function notifyConfidenceEscalation(
    session: Session,
    reactionKey: string,
    action: string,
    threshold: number,
    ...
    const question = buildConfidenceEscalationQuestion(reactionKey, action, threshold, assessment);
    ...
    recordActivityEvent({
      ...
      level: "warn",
      summary: `reaction ${reactionKey} held: confidence ${assessment.score.toFixed(2)} < ${threshold}`,
      data: {
        reactionKey,
        action,
    ...
    const event = createEvent("reaction.escalated", {
      ...
      data: buildReactionEscalationNotificationData({
        ...
        reactionKey,
        action: "escalated",
        attempts: reactionTrackers.get(`${session.id}:${reactionKey}`)?.attempts ?? 0,
        cause: "low_confidence",
        confidence: assessment.score,
    ...
    return notifyHuman(event, priority ?? "urgent");
  }

  /** Persist / clear the reactionKey of an undelivered confidence hold awaiting retry. */
  function setPendingConfidenceHold(session: Session, reactionKey: string): void {
    if (session.metadata[CONFIDENCE_HOLD_PENDING_KEY] === reactionKey) return;
    updateSessionMetadata(session, { [CONFIDENCE_HOLD_PENDING_KEY]: reactionKey });
  }
  function clearPendingConfidenceHold(session: Session, reactionKey: string): void {
    if (session.metadata[CONFIDENCE_HOLD_PENDING_KEY] !== reactionKey) return;
    updateSessionMetadata(session, { [CONFIDENCE_HOLD_PENDING_KEY]: "" });
  }

  /**
   * The SINGLE confidence-hold chokepoint (Class B). Every gated autonomous send
   * routes through here, so a hold reaches EXACTLY ONE terminal outcome:
   *   • proceed        — confidence is sufficient (or the reaction isn't gated).
   *                      Any pending hold is released; the caller acts.
   *   • held-delivered — a human received the question (+ detail). The tracker is
   *                      latched and pending cleared. Re-entry while latched
   *                      short-circuits here (no re-notify).
   *   • held-pending   — held but the escalation could not be delivered. Persisted
   *                      so retryPendingConfidenceHold re-attempts every poll;
   *                      NEVER latched or marked delivered, so a held send is never
   *                      stamped as handled without proof.
   */
  async function withholdForConfidence(
    session: Session,
    reactionKey: string,
    action: string,
    reactionConfig: ReactionConfig,
    detail?: string,
  ): Promise<ConfidenceHoldOutcome> {
    const trackerKey = `${session.id}:${reactionKey}`;
    const existing = reactionTrackers.get(trackerKey);
    if (existing?.escalated && existing.heldForConfidence) {
    ...
    if (delivered) {
      const tracker = existing ?? { attempts: 0, firstTriggered: new Date() };
      tracker.escalated = true;
      tracker.heldForConfidence = true;
      reactionTrackers.set(trackerKey, tracker);
      clearPendingConfidenceHold(session, reactionKey);
    } else {
      setPendingConfidenceHold(session, reactionKey);
    }
    return delivered ? "held-delivered" : "held-pending";
  }
   ...
   * the explicit outcome; stamp the dispatch hash only when the human received it.
   */
  ...
  /** Deliver the standard human-facing form of a notify reaction. Kept separate
   * from executeReaction so pending human outcomes can retry delivery without
   * ever re-entering a disabled autonomous action. */
  async function deliverNotifyReaction(
    session: Session | ReactionSessionContext,
    reactionKey: string,
    reactionConfig: ReactionConfig,
  ): Promise<boolean> {
    ...
    const context = buildEventContext(session, prEnrichmentCache);
    const lifecycleSession = "lifecycle" in session ? session : null;
    const isDecisionReaction =
      !!lifecycleSession &&
      (reactionKey === "agent-needs-input" || reactionKey === "report-needs-input");
    const decision =
      lifecycleSession && isDecisionReaction ? getActiveDecision(lifecycleSession) : null;
    ...
    const expectedDecisionId =
      lifecycleSession && isDecisionReaction ? activeDecisionId(lifecycleSession) : undefined;
    return notifyHuman(event, reactionConfig.priority ?? "info", expectedDecisionId);
  }

  function setPendingDecisionNotification(session: Session, activationIdentity: string): void {
    if (session.metadata[DECISION_NOTIFICATION_PENDING_KEY] === activationIdentity) return;
    updateSessionMetadata(session, {
      [DECISION_NOTIFICATION_PENDING_KEY]: activationIdentity,
    });
  }

  function clearPendingDecisionNotification(session: Session, activationIdentity: string): void {
    if (session.metadata[DECISION_NOTIFICATION_PENDING_KEY] !== activationIdentity) return;
    updateSessionMetadata(session, { [DECISION_NOTIFICATION_PENDING_KEY]: "" });
  }
  ...
  async function retryPendingConfidenceHold(session: Session): Promise<void> {
    ...
    // The operator may disable autonomy after the hold was persisted (or before a
    // restart). Keep retrying the human outcome, but never re-enter the autonomous
    // action: route through the notify-only delivery path instead.
    if (reactionConfig.auto === false && reactionConfig.action !== "notify") {
      const delivered = await deliverNotifyReaction(session, reactionKey, {
        ...reactionConfig,
        action: "notify",
        priority: reactionConfig.priority ?? "urgent",
      });
      if (delivered) clearPendingConfidenceHold(session, reactionKey);
      return;
    }
    await executeReaction(session, reactionKey, reactionConfig);
  }

  /** Retry a failed same-state decision fallback while its exact activation is
   * still current. A successor report is a new activation and owns its own alert. */
  async function retryPendingDecisionNotification(session: Session): Promise<void> {
    const pendingIdentity = session.metadata[DECISION_NOTIFICATION_PENDING_KEY];
    if (!pendingIdentity) return;

    const auditResult = auditAgentReports(session);
    const reportState = auditResult?.report?.state;
    if (
      !auditResult?.trigger ||
      (reportState !== "needs_input" && reportState !== "needs_decision")
    ) {
      clearPendingDecisionNotification(session, pendingIdentity);
      return;
    }
    ...
    if (activationIdentity !== pendingIdentity) {
      clearPendingDecisionNotification(session, pendingIdentity);
      return;
    }

    const decision = reportState === "needs_decision" ? getActiveDecision(session) : null;
    const message =
      reportState === "needs_decision"
        ? decision
          ? formatNeedsDecisionMessage(decision)
          : null
        : auditResult.message;
    ...
    const delivered = await deliverNotifyReaction(
      session,
      getReactionKeyForTrigger(auditResult.trigger),
      { auto: true, action: "notify", priority: "urgent", message },
    );
    if (delivered) clearPendingDecisionNotification(session, pendingIdentity);
  }

  /** The single per-poll retry chokepoint for undelivered human outcomes. */
  async function retryPendingHumanOutcomes(session: Session): Promise<void> {
    await retryPendingConfidenceHold(session);
    await retryPendingDecisionNotification(session);
  }
  ...
  async function executeReaction(
    ...
    const effectiveReactionConfig =
      action === "auto-merge" && reactionConfig.confidenceThreshold === undefined
        ? {
            ...reactionConfig,
            confidenceThreshold: DEFAULT_AUTO_MERGE_CONFIDENCE_THRESHOLD,
          }
        : reactionConfig;

    // Definition-of-done is checked fresh before confidence and before every
    // merge attempt. A status of `mergeable` is only a hint: Codex uses informal
    // approval and COMMENTED reviews, so the SCM's coarse mergeable bit cannot
    // prove the intended reviewer is satisfied or that its threads are resolved.
    let mergeGate: Awaited<ReturnType<typeof evaluateMergeDefinitionOfDone>> | undefined;
    if (action === "auto-merge" && "lifecycle" in session) {
      mergeGate = await evaluateMergeDefinitionOfDone(session, effectiveReactionConfig);
      const nonConfidenceBlockers = mergeGate.blockers.filter(
        (blocker) => blocker !== "low_confidence",
      );
      if (nonConfidenceBlockers.length > 0) {
        ...
        return {
          ...
          success: false,
          action: "auto-merge",
          message: `Definition of done blocked: ${nonConfidenceBlockers.join(", ")}`,
          escalated: false,
        };
      }
      updateAutoMergeBlockers(session, []);
    }
    ...
    switch (action) {
      ...
      case "notify": {
        ...
        // the needs-input notify reaction here. deliverNotifyReaction surfaces the
        // explicit question/confidence and binds controls to this report snapshot.
        await deliverNotifyReaction(session, reactionKey, reactionConfig);
        recordActivityEvent({
          projectId,
          sessionId,
          source: "reaction",
      ...
      case "auto-merge": {
        ...
        try {
          ...
          recordActivityEvent({
            ...
            sessionId,
            source: "reaction",
            kind: "reaction.auto_merge_failed",
            level: "warn",
  ...
  function getReactionConfigForSession(
    ...
    const reactionConfig = projectReaction
      ? mergeReactionOverride(
          globalReaction ?? { auto: true, action: "notify" },
          projectReaction as Partial<ReactionConfig>,
        )
      : globalReaction;
  ...
  function getReviewBotPolicies(session: Session): Record<string, ReviewBotPolicy> | undefined {
    return getReactionConfigForSession(session, "bugbot-comments")?.reviewBots;
  }

  function reviewCommentBotName(comment: ReviewComment): string | undefined {
    return comment.botName ?? (comment.isBot ? comment.author : undefined);
  }

  function reviewSummaryBotName(review: ReviewSummary): string | undefined {
    return review.botName ?? (review.isBot ? review.author : undefined);
  }

  function reviewBotWeight(
    session: Session,
    botName: string,
    legacyActionable = true,
  ): number {
    const policies = getReviewBotPolicies(session);
    const configured = resolveReviewBotPolicy(botName, policies);
    if (configured) return configured.weight;
    // A missing policy preserves the pre-#15 behavior for hand-constructed
    // configs. A configured map without a matching/wildcard entry is fail-safe:
    // only reviewers explicitly targeted by the installation are actionable.
    if (!policies) return legacyActionable ? 1 : 0;
    return 0;
  }
  ...
  function toAutomatedComment(comment: ReviewComment) {
    return {
      id: comment.id,
      botName: reviewCommentBotName(comment) ?? comment.author,
      body: comment.body,
      path: comment.path,
      line: comment.line,
  ...
  async function maybeDispatchReviewBacklog(
    ...
    // Split locally by botName so the configured reviewer policy, not a generic
    // isBot boolean, controls the automated review/fix loop.
    ...
    {
      const unresolved = allThreads.filter((c) => !reviewCommentBotName(c));
      ...
      for (let i = 1; i < sessionPRs.length; i++) {
        ...
        const secondaryUnresolved = secondaryThreads.filter((c) => !reviewCommentBotName(c));
        const secondaryBlob = JSON.stringify({
    ...
    const pendingComments = allThreads.filter((c) => !reviewCommentBotName(c));
    const allAutomatedComments = allThreads.filter((c) => !!reviewCommentBotName(c));
    const automatedById = new Map(allAutomatedComments.map((comment) => [comment.id, comment]));
    const automatedBuckets = partitionAutomatedComments(
      allAutomatedComments.map(toAutomatedComment),
    ...
    const reviewedCurrentHead =
      ...
      reviewSummaries.some((review) => {
        ...
        return (
          !!botName &&
          reviewBotWeight(session, botName, review.isReviewBot === true) >= 1 &&
      ...
      if (!pendingFingerprint) {
        ...
        if (
          ...
          if (hold !== "proceed") {
            ...
            // reaction attempt budget. Only bypass for "send-to-agent"; and NOT
            // when the transition reaction escalated (exhausted retries) — route
            // back through executeReaction so its tracker.escalated guard holds.
            transitionReaction?.key === humanReactionKey &&
            reactionConfig.action === "send-to-agent" &&
      ...
      if (!automatedFingerprint) {
        ...
        if (reviewRounds >= maxReviewRounds) {
          ...
          if (
            ...
            // hold sends ONE escalation (question + comments) to the human. Stamp
            // human or agent delivery for dedup, but count a round ONLY when the
            // payload actually reaches the agent.
            const hold =
              reactionConfig.action === "send-to-agent"
                ? await withholdDispatchForConfidence(
                    session,
            ...
            if (hold !== "proceed") {
              ...
              // tracker.escalated guard keeps it held.
              transitionReaction?.key === automatedReactionKey &&
              reactionConfig.action === "send-to-agent" &&
              !transitionReaction?.result?.escalated
            ) {
  ...
  async function maybeNudgeStuckAgent(
    ...
    if (automatedAlreadyDelivered) {
      parts.push(formatAutomatedReviewComments(session, automatedComments));
    }
  ...
  async function primaryPRCIGreen(session: Session, scm: SCM): Promise<boolean> {
    ...
    if (!p) return false;
    const cached = prEnrichmentCache.get(`${p.owner}/${p.repo}#${p.number}`);
    const ci = cached ? cached.ciStatus : await scm.getCISummary(p).catch(() => undefined); // live fallback
  ...
  function reviewBodyMatchesApproval(body: string, policy: ReviewBotPolicy): boolean {
    ...
    return (policy.approvalPhrases ?? []).some((phrase) =>
      normalizedBody.includes(phrase.toLowerCase().replace(/\s+/g, " ")),
    );
  }
  ...
  async function evaluateMergeDefinitionOfDone(
    ...
    for (const pr of prs) {
      try {
        const [prState, ciStatus, mergeability, reviewDecision, reviewData] = await Promise.all([
          scm.getPRState(pr),
          scm.getCISummary(pr),
          scm.getMergeability(pr),
          scm.getReviewDecision(pr),
          scm.getReviewThreads(pr, { forceFresh: true }),
        ]);
        ...
        const unresolvedRequiredThreads = reviewData.threads.filter((comment) => {
          ...
          return !botName || reviewBotWeight(session, botName) >= 1;
        }).length;

        const botReviewApproved = reviewData.reviews.some((review) => {
          const botName = reviewSummaryBotName(review);
          if (
            !botName ||
            reviewBotWeight(session, botName, review.isReviewBot === true) < 1
          ...
          const currentHeadReview = !!reviewData.headSha && review.commitSha === reviewData.headSha;
          if (!currentHeadReview) return false;
          return (
        ...
        // Every SCM read is decision-critical. A transient failure must never be
        // interpreted as done.
        blockers.add("review_data_incomplete");
      }
    }
  ...
  function updateAutoMergeBlockers(session: Session, blockers: string[]): void {
    const next = blockers.join(",");
    if ((session.metadata["autoMergeBlockers"] ?? "") === next) return;
    updateSessionMetadata(session, { autoMergeBlockers: next });
  }

  function actionableRebumpPolicy(
    session: Session,
  ): { botName: string; policy: ReviewBotPolicy } | null {
    const policies = getReviewBotPolicies(session);
    if (!policies) return null;
    for (const [botName, policy] of Object.entries(policies)) {
      if (botName !== "*" && policy.weight >= 1 && policy.rebumpMessage) {
        return { botName, policy };
      }
    }
    return null;
  }
  ...
  async function maybeAutoMergeWhenDone(session: Session, newStatus: SessionStatus): Promise<void> {
    if (!session.pr || session.lifecycle.pr.state !== "open") return;
    if (session.metadata["autoMergeRequestedAt"]) return;
    const reactionConfig = getReactionConfigForSession(session, "approved-and-green");
    if (
    ...
    await executeReaction(session, "approved-and-green", reactionConfig);
  }
  ...
  /**
   * Retry classifier-approved flaky CI before the ci-failed reaction is allowed
   * to consume an agent fix attempt. Returns true while the retry grace window
   * owns this exact failure fingerprint.
   */
  async function maybeRetryFlakyCI(session: Session, newStatus: SessionStatus): Promise<boolean> {
    if (newStatus !== "ci_failed" || !session.pr) return false;
    const reactionConfig = getReactionConfigForSession(session, "ci-failed");
    const maxRetries = reactionConfig?.flakyRetries ?? 0;
    if (maxRetries <= 0) return false;

    const project = config.projects[session.projectId];
    const scm = project?.scm?.plugin ? registry.get<SCM>("scm", project.scm.plugin) : null;
    if (!scm?.retryCI) return false;
    const failedChecks = await getFailedCIChecks(scm, session.pr, { allowFetch: true });
    if (!failedChecks) return false;

    const fingerprint = makeCIFailureFingerprint(failedChecks);
    const lastFingerprint = session.metadata["lastFlakyCIFingerprint"] ?? "";
    const lastRetryAt = Date.parse(session.metadata["lastFlakyCIRetryAt"] ?? "");
    const graceMs = parseDuration(reactionConfig?.flakyRetryBackoff ?? "10m");
    if (
      fingerprint === lastFingerprint &&
      Number.isFinite(lastRetryAt) &&
      Date.now() - lastRetryAt < graceMs
    ) {
      return true;
    }

    const retryCount = parseAttemptCount(session.metadata["flakyCIRetryCount"]);
    if (retryCount >= maxRetries) return false;

    let summary: CIFailureSummary | null = null;
    if (scm.getCIFailureSummary) {
      try {
        summary = await scm.getCIFailureSummary(session.pr, failedChecks);
      } catch {
        // Check conclusions alone may still identify a flaky infrastructure run.
      }
    }
    const classification = classifyCIFailure(failedChecks, summary);
    if (classification.kind !== "flaky") return false;

    try {
      if (!(await scm.retryCI(session.pr, failedChecks))) return false;
    } catch {
      return false;
    }

    updateSessionMetadata(session, {
      lastFlakyCIFingerprint: fingerprint,
      lastFlakyCIRetryAt: new Date().toISOString(),
      flakyCIRetryCount: String(retryCount + 1),
    });
    recordActivityEvent({
      projectId: session.projectId,
      sessionId: session.id,
      source: "reaction",
      kind: "ci.flaky_retry",
      summary: `retried flaky CI before nudging the agent: ${classification.reason}`,
      data: {
  ...
  async function maybeDispatchCIFailureDetails(
    ...
    if (ciFingerprint !== lastCIFingerprint) {
      // Count each distinct failed-check set as CI churn. Cumulative — survives CI
      // recovery, reset only at a terminal PR state (#12 review).
      updateSessionMetadata(session, {
        lastCIFailureFingerprint: ciFingerprint,
        ciFailureCountTotal: String(parseAttemptCount(session.metadata["ciFailureCountTotal"]) + 1),
      });
    }
  ...
  async function checkSession(
    ...
    if (evaluateBudget) {
      ...
      if (breach) {
        ...
        const pausedAt = firstPause ? new Date().toISOString() : session.metadata["budgetPausedAt"];
        ...
        const costObserved =
          projectCostUsd !== undefined || session.agentInfo?.cost?.estimatedCostUsd !== undefined;
    ...
    // Classify before transition reactions. When this returns true, the SCM retry
    // owns the incident and neither executeReaction nor the detailed dispatcher
    // may spend an agent fix round for it.
    const flakyCIRetryPending = await maybeRetryFlakyCI(session, newStatus);
    ...
    if (newStatus !== oldStatus) {
      ...
      // A pending confidence hold is scoped to the status that triggered it; once
      // the status changes the held condition is stale, so release the retry marker
      // (a fresh hold re-arms on the new status if still warranted) (#12 review).
      if (session.metadata[CONFIDENCE_HOLD_PENDING_KEY]) {
        clearPendingConfidenceHold(session, session.metadata[CONFIDENCE_HOLD_PENDING_KEY]);
      }
      ...
      if (eventType) {
        ...
        if (reactionKey && !suppressContextOnlyChangesRequested) {
          ...
          if (reactionConfig && reactionConfig.action) {
            ...
            if (reactionConfig.auto !== false || reactionConfig.action === "notify") {
              // Auto-merge is level-triggered after the review/CI dispatchers have
              // refreshed their facts below. Deferring avoids racing a stale
              // mergeable transition against a newly-opened review thread.
              if (reactionKey === "ci-failed" && flakyCIRetryPending) {
                reactionHandledNotify = true;
              } else if (reactionConfig.action === "auto-merge") {
                reactionHandledNotify = true;
              } else {
    ...
    await maybeAutoMergeWhenDone(session, newStatus);
  ...
  async function auditAndReactToReports(
    ...
    // Retry every undelivered human outcome through one per-poll chokepoint:
    // confidence holds and same-state decision notifications.
    await retryPendingHumanOutcomes(session);
    const auditResult = auditAgentReports(session);
    ...
    if (
      ...
      const decision = decisionReportState === "needs_decision" ? getActiveDecision(session) : null;
      const message =
      ...
      if (message !== null) {
        ...
        if (delivered) {
          clearPendingDecisionNotification(session, activationIdentity);
        } else {
          setPendingDecisionNotification(session, activationIdentity);
        }
      }
    }
  }
  ...
  async function pollAll(): Promise<void> {
    ...
    try {
      ...
      const queueProjectIds = scopedProjectId ? [scopedProjectId] : Object.keys(config.projects);
  ...
  return {
    ...
    async check(sessionId: SessionId, options: LifecycleCheckOptions = {}): Promise<void> {
      ...
      await checkSession(session, projectCostUsd.get(session.projectId), scmRefreshOutcomes);
