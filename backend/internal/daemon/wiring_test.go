...
func TestWiring_StartSessionBuildsSessionService(t *testing.T) {
	...
	svc, reviewSvc, lc, err := startSession(context.Background(), cfg, rt, store, lcm, messenger, telemetryadapter.NoopSink{}, log)
	if err != nil {
...
func TestStartSession_SpawnDoesNotPanicWhenNoTrackerToken(t *testing.T) {
	...
	rt := runtimeselect.New(nil)
	messenger := newSessionMessenger(store, rt, log)
	svc, _, _, err := startSession(context.Background(), cfg, rt, store, lcm, messenger, telemetryadapter.NoopSink{}, log)
	if err != nil {
		t.Fatalf("startSession: %v", err)
	}
