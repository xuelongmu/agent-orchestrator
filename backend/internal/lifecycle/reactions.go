...
func (m *Manager) writeDesignContractDispatch(ctx context.Context, msg *strings.Builder, workspacePath, prURL string) error {
	...
	msg.WriteString("Before pushing the review-fix commit, use `ao verify <configured-profile>` for applicable configured verification that could fan out or destabilize the agent terminal, and read the printed log. Never guess a profile or bypass repository verification rules.\n")
	return nil
}
