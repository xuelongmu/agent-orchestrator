...
func genPRParams(r domain.PullRequest) gen.UpsertPRParams {
	return gen.UpsertPRParams{
		...
		Repo:                     r.Repo,
		HeadRepo:                 r.HeadRepo,
		StackNumber:              int64(r.StackNumber),
		StackPosition:            int64(r.StackPosition),
		StackSize:                int64(r.StackSize),
...
func prRowFromGen(p gen.PR) domain.PullRequest {
	return domain.PullRequest{
		...
		Repo:                     p.Repo,
		HeadRepo:                 p.HeadRepo,
		StackNumber:              int(p.StackNumber),
		StackPosition:            int(p.StackPosition),
		StackSize:                int(p.StackSize),
