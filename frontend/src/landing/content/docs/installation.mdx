...
| Node.js 20+     | npm/source installs    | Needed only if you use the npm bootstrap or build AO from source.                    |
...
<summary>
	<span className="ao-legacy-cli-summary-copy">
		...
		<span className="ao-legacy-cli-summary-note">npm still works but is no longer recommended. Verify the <code>@aoagents/ao</code> bootstrap, or move to the desktop build.</span>
	</span>
...
npm still works but is no longer recommended. `@aoagents/ao` is a bootstrap that launches the matching platform Go binary; the historical `@aoagents/ao-cli` package is the legacy TypeScript runtime. These commands remain for existing users; new setups should use the desktop download at the top of this page.
