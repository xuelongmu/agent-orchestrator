...
<Accordion title="Can I still install AO via npm?">
  ...
  <Accordion title="How do I completely uninstall?">
    `npm uninstall -g @aoagents/ao`, then `rm -rf ~/.ao` if you want to wipe all state stored in the default
    location. If you set `AO_DATA_DIR`, remove that directory separately; if you set `AO_RUN_FILE`, remove that file too.
  </Accordion>
</Accordions>
