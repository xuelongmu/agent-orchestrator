...
<Accordion title="Where does session state live?">
	AO keeps durable session and activity state in SQLite under `~/.ao/data`. Worktrees, the daemon's `running.json`
	handshake, and Electron profile data are also kept under `~/.ao`; transient runtime state stays in memory. Set
	`AO_DATA_DIR` to override the daemon data directory.
</Accordion>
...
<Accordion title="Can I still install AO via npm?">
  ...
  <Accordion title="How do I completely uninstall?">
    ...
    location. If you set `AO_DATA_DIR`, remove that directory separately.
  </Accordion>
</Accordions>
