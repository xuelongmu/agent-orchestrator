...
Agent Orchestrator (AO) is a Node.js orchestrator that spawns and manages parallel AI coding agents across dedicated git worktrees. Every moving part is a plugin; the core provides the state machine, event bus, and prompt assembly that ties them together.
...
| Workspace | [worktree](/docs/plugins/workspaces/worktree)   | Code separation — each session gets its own git worktree or clone | `Workspace`        |
