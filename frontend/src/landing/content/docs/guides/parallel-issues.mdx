...
```bash
gh issue view 43 --json blockedBy --jq '.blockedBy.nodes[] | select(.state == "OPEN") | .number'
```

An empty result means #43 has no open blockers — spawn it. If it lists issue numbers, spawn those first and hold #43 until they close. Batch-spawn each "wave" of unblocked issues, then batch-spawn the next wave as blockers merge.
