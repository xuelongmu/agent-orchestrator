...
## npm is a bootstrap and no longer recommended

npm still works, but it is no longer the recommended way to install AO. `@aoagents/ao` is a bootstrap that launches the matching platform Go binary; the historical `@aoagents/ao-cli` package is the legacy TypeScript runtime. Move to the [desktop app](https://ao-agents.com/docs/installation), which auto-updates and owns the daemon.
